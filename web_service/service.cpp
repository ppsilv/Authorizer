...
const std::shared_ptr<httpserver::http_response> service_resource::render_POST(const httpserver::http_request &req) {
    ...
    //if (verbose) std::cout << "req  ***"<< req << "***\n";

    std::string h1 = req.get_header("merda");
   std::cout << "H1  [" << h1 << "]\n";

    std::string datapar = req.get_content();
    std::cout << "Dados [" << datapar << "]\n";

    httpserver::string_response* res = new httpserver::string_response("POST response: Transaction received", 200);
...
void usage() {
    std::cout << "Usage:" << std::endl << "service [-p <port>][-s [-k <keyFileName>][-c <certFileName>]][-v]" << std::endl;
}
